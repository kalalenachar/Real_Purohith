      ...
      <div className="modal-box" style={{ maxWidth: 600 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div className="modal-body-scroll">
            {/* Avg Rating Display */}
            <div style={{ padding: '14px 18px', borderRadius: 14, background: 'rgba(245,158,11,0.07)', border: '1px solid rgba(245,158,11,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: 12, color: '#94a3b8' }}>Your Overall Rating</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {[1,2,3,4,5].map(s => (
                  <Star key={s} size={16} style={{ color: s <= Math.round(+avgRating) ? '#fbbf24' : '#1e293b', fill: s <= Math.round(+avgRating) ? '#fbbf24' : '#1e293b' }} />
                ))}
                <span className="text-gold-gradient" style={{ fontSize: 20, fontWeight: 900, fontFamily: 'Outfit,sans-serif', marginLeft: 6 }}>{avgRating}</span>
              </div>
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {PARAMS.map(p => (
                  <div key={p.key} className="param-row">
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 600, color: '#e2e8f0' }}>{p.emoji} {p.title}</div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                      {[1,2,3,4,5].map(s => (
                        <button type="button" key={s} className="star-btn" onClick={() => setRatings(r => ({ ...r, [p.key]: s }))}>
                          <Star size={18} className={s <= ratings[p.key] ? 'star-filled' : 'star-empty'}
                            style={{ fill: s <= ratings[p.key] ? '#fbbf24' : '#1e293b', color: s <= ratings[p.key] ? '#fbbf24' : '#1e293b' }} />
                        </button>
                      ))}
                      <span style={{ fontSize: 12, fontWeight: 700, color: '#fbbf24', marginLeft: 6, fontFamily: 'monospace', minWidth: 14 }}>
                        {ratings[p.key]}
                      </span>
                    </div>
                  </div>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                <label style={{ fontSize: 11, fontWeight: 600, color: '#94a3b8' }}>Detailed Feedback</label>
                <button type="button" onClick={handleVoice} style={{
                  ...
                  {recording ? <Mic size={12} /> : voiceDone ? <CheckCircle2 size={12} /> : <Mic size={12} />}
            ...
            {/* notice */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 12, background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.2)', fontSize: 11, color: '#c4b5fd' }}>
              <Sparkles size={14} style={{ flexShrink: 0, color: '#a78bfa' }} />
